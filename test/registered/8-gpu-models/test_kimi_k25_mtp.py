...
AVG_SPEC_ACCEPT_LENGTH_THRESHOLD = 2.5


class TestKimiK25TPMTP(CustomTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_a_gsm8k(self):
        """GSM8K evaluation — named with 'a' prefix to run first and warm up the server."""
    ...
    def test_bs_1_speed(self):
        ...
        if is_in_ci():
            write_github_step_summary(
                f"### test_bs_1_speed (kimi-k25 MTP)\n"
                f"{acc_length=:.2f}\n"
                f"{speed=:.2f} token/s\n"
            )
