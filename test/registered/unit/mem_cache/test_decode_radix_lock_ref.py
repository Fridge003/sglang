...
class TestDecodeBatchEviction(unittest.TestCase):
    ...
    def test_pop_preallocated_keeps_suffix_when_batch_evict_is_insufficient(self):
        queue = DecodePreallocQueue.__new__(DecodePreallocQueue)

        available_tokens = {"value": 0}
        evictable_tokens = {"value": 6}
        req_to_token = torch.full((2, 8), -1, dtype=torch.int64)
        ...
        metadata_alloc = MagicMock()
        metadata_alloc.available_size.return_value = 8
        metadata_alloc.alloc.side_effect = [0]
        ...
        tree_cache = MagicMock()
        tree_cache.evictable_size.side_effect = lambda: evictable_tokens["value"]
        tree_cache.protected_size.return_value = 0

        def _evict(params):
            evicted = min(params.num_tokens, evictable_tokens["value"])
            evictable_tokens["value"] -= evicted
            available_tokens["value"] += evicted
            return MagicMock(num_tokens_evicted=evicted)
        ...
        def _pre_alloc(req, prefix_indices=None, prefix_len=None, *, allow_evict=True):
            kv_loc = torch.tensor([0, 1, 2, 3], dtype=torch.int64)
            available_tokens["value"] -= len(kv_loc)
            req_to_token[req.req_pool_idx, :4] = kv_loc
            return kv_loc
        ...
        self.assertEqual(preallocated[0].req.rid, "r1")
        self.assertEqual(len(failed), 0)
        self.assertEqual(tree_cache.evict.call_count, 1)
        self.assertEqual(tree_cache.evict.call_args.args[0].num_tokens, 8)
        self.assertEqual(len(queue.queue), 1)
        self.assertEqual(queue.queue[0].req.rid, "r2")
        preallocated[0].kv_receiver.send_metadata.assert_called_once()
