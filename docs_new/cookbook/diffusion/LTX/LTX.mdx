...
The LTX series supports one-stage, two-stage, and HQ two-stage pipelines. The recommended launch configuration depends on whether the target GPU can keep both two-stage DiTs resident.
...
- `--pin-cpu-memory`: Recommended with LTX deployment because SGLang uses CPU offload paths for large video components.
...
#### 4.1.3 LTX-2.3 two-stage text-to-video
...
#### 4.1.4 LTX-2.3 HQ text-to-video
...
#### 4.1.5 Image-to-video with one reference image
...
#### 4.1.6 First-to-last-frame transition with two reference images
