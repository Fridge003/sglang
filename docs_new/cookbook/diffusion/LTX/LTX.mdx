...
LTX-2 and LTX-2.3 are released under the LTX-2 Community License Agreement, which includes commercial-use restrictions for some entities; review the [official Lightricks license](https://huggingface.co/Lightricks/LTX-2.3/blob/main/LICENSE) before production or commercial use.
...
### 3.1 Fastest launch on H200 or high-memory CUDA GPUs

For a single NVIDIA H200, or another CUDA GPU with at least 130 GiB of VRAM, keep both two-stage DiTs resident on GPU. This is the fastest serving mode because stage switching does not need to reload transformer weights.
...
Use the same deployment mode for the HQ pipeline when you want the higher-resolution two-stage path:
...
To preload the Transition LoRA on the same H200-class environment, add only the LoRA selector:
...
### 3.2 Standard CUDA GPUs

Use `snapshot` mode when the GPU cannot keep both two-stage DiTs resident. It is the default on lower-VRAM CUDA GPUs and prefetches stage-2 weights when possible.
...
### 3.3 Official switching semantics

Use `original` mode when validating against the original LTX two-stage loading behavior:
