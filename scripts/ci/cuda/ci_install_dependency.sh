...
#
# Each step is a self-contained function called from main() at the bottom.
# Functions communicate via global variables declared in setup_configuration().
set -euxo pipefail
...
setup_configuration() {
    ...
    # Whether to create a uv venv. Default false; set USE_VENV=1 to create one.
    USE_VENV="${USE_VENV:-0}"
...
install_sglang_kernel() {
    ...
    # Reinstall torch packages if CUDA version doesn't match
    ...
    # Reinstall sglang-kernel if CUDA version tag doesn't match
...
stabilize_flashinfer_jit_paths() {
    ...
    # Clear stale cached_ops whose build.ninja references dead venv paths
    ...
    # Copy source files to a stable host-mounted path and symlink the venv's copy
