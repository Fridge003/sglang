...
# Hash-based skip: if python/pyproject.toml hasn't changed since the last
# successful full install, skip dependency resolution (--no-deps) to save
# ~30-100s.  All other steps (apt, flashinfer, sglang-kernel, fix deps, etc.)
# still run unconditionally, so the environment stays correct.
EXTRAS="dev"
