...
def update_est_times(medians, dry_run=False):
    ...
    Each registration call is matched by both the backend (from the function
    name) and suite, so files with multiple registrations for different suites
    get the correct per-suite median.
